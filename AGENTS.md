...
- **Unit tested**, table-driven via `t.Run` subtests. Stdlib `testing` is the
  ...
  when a table's per-case assertions get repetitive. `go test -race` is the
  floor (`mise run test`); don't submit goroutine-touching code you haven't
  run under `-race`.
...
- **`github.com/spf13/pflag`, only when the app has a real CLI surface**:
  ...
  serve." This repo is server/webhook-shaped (`internal/server`), not
  flag-driven; wire pflag through `github.com/spf13/cobra` (see `chaski`,
  `flate`, `ocharted`) only if a real CLI surface is added, rather than a
  bare `pflag.FlagSet`.
...
This repo also has `mise run fmt-check`, `tidy-check`, `test-integration`,
`generate`/`generate-check`, `dev`, and a `ui-*` family (`ui-install`,
`ui-typecheck`, `ui-build`, `ui-test`) for its frontend; check `mise tasks`
...
OOM-kills the process (this repo already does, see `main.go`); expose
