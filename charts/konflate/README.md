...
![Version](https://img.shields.io/static/v1?label=Version&message=0.6.4&color=informational&style=flat-square) <!-- x-release-please-version -->
![Type: application](https://img.shields.io/badge/Type-application-informational?style=flat-square)
![AppVersion](https://img.shields.io/static/v1?label=AppVersion&message=0.6.4&color=informational&style=flat-square) <!-- x-release-please-version -->
...
| config.forceGenericProvider | string | `""` | Route a `spec.provider` source (GitRepository/OCIRepository/Bucket set to `gcp`/`aws`/`azure` for cloud IAM auth in the live cluster) through flate's generic SecretRef/`--registry-config` credential path instead of failing — flate can't perform cloud IAM auth offline. Empty = the opposite of `renderForkPrs` (on unless rendering untrusted forks, since this makes flate authenticate with a real credential against whatever host a PR's manifest names). `"true"` enables it outright; `"false"` disables it even when not rendering forks. |
| config.helmApiVersions | string | `""` | Extra API versions charts see in `.Capabilities.APIVersions`, comma-separated (e.g. `"resource.k8s.io/v1/DeviceClass"`). Appended to helm's built-in set, so list only what the target cluster serves beyond the core APIs: a chart gated on a CRD or feature-gated API (`.Capabilities.APIVersions.Has`) otherwise fails or renders differently than in-cluster, since konflate templates offline. Set `"{{ join \",\" .Capabilities.APIVersions }}"` to track the cluster this release installs into (only correct when it is also the deploy target). |
...
| config.publicUrl | string | `""` | konflate's externally-reachable base URL (e.g. https://konflate.example.com); a posted status/comment links back to it. Empty = posted without a link. `basePath` is appended automatically when it is set and not already present. |
