...
| config.basePath | string | `""` | URL path prefix konflate is served under (e.g. /platform/konflate). Empty = root. The app registers every route on the main listener under this prefix; configure your ingress to forward requests with the prefix preserved. |
