...
| `KONFLATE_METRICS_ADDR`             | `:8081`       | Listen address for the **separate** monitoring server — `/metrics` plus the `/healthz` / `/readyz` probes. Bind to loopback to keep it private.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
...
| `GET /healthz`, `GET /readyz` | Liveness / readiness. Also served on the monitoring port (see below).                                                                                                                                                                                                                                                                                                                                                                                                                                          |

Monitoring server (`KONFLATE_METRICS_ADDR`, default `:8081`): `GET /metrics` plus the `GET /healthz` / `GET /readyz` probes — kept off the main, possibly public-facing port. The chart's probes target this port.
...
Served on the separate monitoring port (`KONFLATE_METRICS_ADDR`, default `:8081`,
alongside the health/readiness probes — keep it off your public ingress):
