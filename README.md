...
| `KONFLATE_KUBE_VERSION`             | _(none)_      | Kubernetes version charts see as `.Capabilities.KubeVersion`. Empty = the version bundled with the helm SDK konflate was built against, which may be newer than your cluster, so a chart gated on the cluster version renders differently here than in-cluster. Set it to the target cluster's version (e.g. `1.33.4`). With the Helm chart, `config.kubeVersion` accepts `"{{ .Capabilities.KubeVersion.Version }}"` to track the cluster the release installs into.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| `KONFLATE_HELM_API_VERSIONS`        | _(none)_      | Extra API versions charts see in `.Capabilities.APIVersions`, comma-separated (e.g. `resource.k8s.io/v1/DeviceClass`). Appended to helm's built-in set, so list only what the target cluster serves beyond the core APIs: a chart gated on a CRD or feature-gated API (`.Capabilities.APIVersions.Has`) otherwise fails or renders differently than in-cluster, since konflate templates offline with no API server to discover them from. With the Helm chart, `config.helmApiVersions` accepts `"{{ join \",\" .Capabilities.APIVersions }}"` to track the cluster the release installs into (only correct when it is also the deploy target).                                                                                                                                                                                                                                                                                                                                                                                    |
...
| `KONFLATE_COMMENT_TAG`              | _(none)_      | Disambiguates this instance's hidden comment marker when several konflate instances sharing one bot identity render the same PR (a folder-per-cluster monorepo), so each keeps its own comment instead of overwriting the other's. Empty falls back to a custom `KONFLATE_STATUS_CHECK_NAME`, then to no tag (the marker every prior release wrote). See [Write-back](#write-back).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
...
| `KONFLATE_PUBLIC_URL`               | _(none)_      | konflate's externally-reachable base URL, e.g. `https://konflate.example.com`. Used only to build the review link a posted status/comment points back to; unset ⇒ posted with no link. `basePath` is appended automatically when it is set and not already present.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| `KONFLATE_BASE_PATH`                | _(root)_      | URL path prefix konflate is served under, e.g. `/platform/konflate`. Empty = root. The server registers every route on the main listener under this prefix, injects it into the SPA, and prefixes internally-generated URLs. The reverse proxy must forward requests with the prefix preserved.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| `KONFLATE_MCP`                      | `false`       | Serve a read-only [MCP](https://modelcontextprotocol.io) endpoint at `/mcp` so an AI agent can query konflate's rendered-diff analysis (open PRs + their summaries) via the `list_pull_requests` / `get_pr_summary` / `get_pr_diff` tools. Read-only — the same data as the API, triggering no render or write. Off by default (a new surface); secure it at your ingress if exposed. See [HTTP endpoints](#http-endpoints).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| `KONFLATE_VERIFY_IMAGES`            | `false`       | Check that each container image a PR **newly references** exists in its registry (a HEAD on the tag/digest), mark each image change `found` / `not found` / `unverified` in the review and the PR comment, and raise an `image-not-found` **blocker** (attributed to each workload that pulls it; fails the check) for any that's absent — catching a typo'd or not-yet-pushed image before it `ImagePullBackOff`s in-cluster. Auth/network errors are skipped (never a false "missing"; the image stays `unverified`). ⚠️ Only **trusted (non-fork)** PRs are checked: a fork's images are attacker-chosen, so dialing a registry named there is an SSRF vector — fork verification (behind an egress allowlist) is a deliberate follow-up. **Private registries:** mount a `dockerconfigjson` and point `DOCKER_CONFIG` at it (go-containerregistry's keychain is host-scoped, so creds never leak cross-registry); konflate never reads a manifest's `imagePullSecrets` (it renders YAML and holds no cluster/decryption state). |
| `KONFLATE_IMAGE_VERIFY_TIMEOUT`     | `5s`          | Per-image registry-check timeout (see `KONFLATE_VERIFY_IMAGES`). The whole step is also bounded by `KONFLATE_DIFF_TIMEOUT`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| `KONFLATE_PORT`                     | `8080`        | Main HTTP port (UI, API, websocket, webhook).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| `KONFLATE_METRICS_ENABLED`          | `true`        | Serve Prometheus `/metrics` on the separate metrics port; `false` removes that listener entirely (probes are unaffected — they ride the main port).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| `KONFLATE_METRICS_PORT`             | `8081`        | Metrics listen port (`/metrics` only), kept off the main, possibly public-facing port.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
...
| **blocking** | red    | PRs with a finding that means the change cannot deploy — today an image reference its registry lacks (`KONFLATE_VERIFY_IMAGES`). Fails the check.                                                                                    |
| **caution**  | amber  | PRs carrying a heuristic danger flag — data-loss, privilege, RBAC, availability, or a major version bump. Advisory only; a caution never fails the check.                                                                            |
...
Running several konflate instances against one repo (one per cluster folder of
a monorepo, sharing a bot identity)? Give each a distinct `KONFLATE_COMMENT_TAG`
so they keep separate comments on the same PR; unset, a custom
`KONFLATE_STATUS_CHECK_NAME` doubles as the tag.
...
rendered summary (blockers, cautions, render failures, blast radius, image
changes) in the PR's Checks tab, which you can require as a merge gate (cautions
are a non-blocking _neutral_; a blocker or render failure is a _failure_). A write PAT, GitLab, Forgejo, or an App without
...
| Forgejo / Gitea | `KONFLATE_WRITE_TOKEN`                                                 | Token with `write:repository` (statuses) and, for comments, `write:issue` plus `read:user` (konflate looks up the token's own user to find its comment). A token restricted to **Specific repositories** can't carry `read:user`, so it works for statuses only. |
...
| `.Sections`  | the summary's blocks _individually_, as rendered Markdown — `.Sections.Impact`, `.Sections.Failing` (blockers + render failures, one red box on GitHub), `.Sections.Blocking`, `.Sections.Cautions`, `.Sections.Failures`, `.Sections.Images`, `.Sections.BlastRadius` — to place à la carte instead of the whole `.Summary`. Each is empty when that block has nothing to show. |
...
| `/mcp`                        | **Opt-in** (`KONFLATE_MCP=true`) — a read-only [MCP](https://modelcontextprotocol.io) endpoint (streamable HTTP). Absent unless enabled.                                                                                                                                                                                                                                                                                                                                                                       |
| `GET /healthz`, `GET /readyz` | Liveness / readiness (the chart's probes target these on the main port).                                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
The comment carries a `<!-- konflate:pr-N -->` marker (with a hash of
`KONFLATE_COMMENT_TAG` appended when one is set) so a poster can find and update
its own comment. konflate keeps PRs rendered on its own (webhook /
...
Served on the separate metrics port (`KONFLATE_METRICS_PORT`, default `8081` —
keep it off your public ingress; health probes ride the main port):
