...
now recommends — no numeric app id needed.) A fully-configured App takes
precedence over `KONFLATE_WRITE_TOKEN`; a partial App config is a startup error
rather than a silent fallback. The write token is the universal option and the
only one on GitLab and Forgejo — keep it separate from `KONFLATE_TOKEN` so it
