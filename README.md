...
| `KONFLATE_PR_LABELS`        | _(all PRs)_   | Comma-separated label allowlist. When set, only PRs carrying at least one of these labels are tracked; the rest are ignored. Matched case-insensitively (`cluster` or `cluster:production` both work). Empty tracks every open PR.                                                                                                                                                                                                                                                                                                                                  |
| `KONFLATE_PR_FILTER_EXPR`   | _(no filter)_ | [CEL](https://cel.dev) expression for fine-grained PR selection beyond the label allowlist — negation, author/base rules, drafts, combinations. Evaluates against a `pr` variable (fields: `number`, `title`, `author`, `draft`, `open`, `merged`, `state`, `fork`, `headRef`, `headSha`, `baseRef`, `url`, `createdAt`, and `labels` as `[{name, color}]`) and must return a boolean. Compiled and type-checked at startup. Composes with `KONFLATE_PR_LABELS` — a PR must pass both. Example: `pr.labels.exists(l, l.name == "cluster/production") && !pr.draft`. |
