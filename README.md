...
| `KONFLATE_PR_FILTER_EXPR`   | `!pr.fork`    | [CEL](https://cel.dev) expression deciding which PRs konflate **renders** (and shows by default). PRs it excludes are still listed — greyed, under a "hidden" pill — but never rendered. Evaluates against a `pr` variable (fields: `number`, `title`, `author`, `draft`, `open`, `merged`, `state`, `fork`, `headRef`, `headSha`, `baseRef`, `url`, `createdAt`, and `labels` as `[{name, color}]`) and must return a boolean; compiled and type-checked at startup. **Empty defaults to `!pr.fork`** — every open PR except forks. ⚠️ Rendering a fork runs untrusted external code (SSRF / resource-exhaustion surface), so admitting a fork in the expression renders it — see [Filtering & forks](#filtering--forks). Example: `pr.labels.exists(l, l.name == "cluster/production") && !pr.draft && !pr.fork`. |
...
`KONFLATE_PR_FILTER_EXPR` is the single switch for which PRs konflate **renders**. It's a [CEL](https://cel.dev) boolean over the `pr` fields above, compiled and type-checked at startup (a malformed expression fails fast). A PR the expression excludes is still tracked and listed — greyed, grouped under a **"hidden"** pill, and kept out of the default view — but is **never rendered**, so its code never runs.

The default, `!pr.fork`, renders every open PR **except forks**. Forks are excluded out of the box because rendering one runs untrusted external code through flate — it fetches whatever sources the fork declares (SSRF, resource exhaustion). Excluded forks still appear under the "hidden" pill (metadata only); they're just never rendered. Set your own expression to narrow what's rendered:

```bash
# only PRs labelled for the production cluster, excluding drafts and forks
KONFLATE_PR_FILTER_EXPR='pr.labels.exists(l, l.name == "cluster/production") && !pr.draft && !pr.fork'
# everything except Renovate's PRs, on the main branch
KONFLATE_PR_FILTER_EXPR='pr.author != "renovate[bot]" && pr.baseRef == "main" && !pr.fork'
```

**To render fork PRs, admit them in the expression** — drop the `!pr.fork` clause, or scope it to trusted authors:

```bash
# ⚠️ renders untrusted external code from EVERY fork PR
KONFLATE_PR_FILTER_EXPR='true'
# same-repo PRs plus forks from one known contributor
KONFLATE_PR_FILTER_EXPR='!pr.fork || pr.author == "trusted-contributor"'
```

When the expression you set would admit a fork, konflate logs a warning at startup. Keep forks excluded (`!pr.fork`) on any public or shared instance.
