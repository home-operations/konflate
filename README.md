...
| `KONFLATE_GIT_DEPTH`                | `0`           | Shallow-clone depth for the persistent PR mirror. **Default `0` = full history (the only safe value today).** A positive value is currently broken: when a PR's merge-base lies beyond the shallow boundary (common on an active base branch) go-git's merge-base errors and the mirror re-clone-loops, failing every render — leave it `0` until shallow is reworked. Distinct from flate's per-source clone depth.                                                                                                                                                                                                                                                                                                                                    |
