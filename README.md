...
| `.Sections`  | the summary's blocks _individually_, as rendered Markdown — `.Sections.Impact`, `.Sections.Blocking`, `.Sections.Cautions`, `.Sections.Failures`, `.Sections.Images`, `.Sections.BlastRadius` — to place à la carte instead of the whole `.Summary`. Each is empty when that block has nothing to show. |
