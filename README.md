...
| `KONFLATE_GIT_DEPTH`                | `50`          | Shallow-clone depth for the persistent PR mirror — a small depth makes the cold clone of a large repo far cheaper (only recent history transfers, lower peak memory). konflate deepens automatically when a PR's merge-base lies further back, so the diff is still computed against the true merge-base. The mirror persists on `KONFLATE_CACHE_DIR`, so this mainly helps a fresh volume (first deploy, or no persistence). `0` = full history. Distinct from flate's per-source clone depth.                                                                                                                                                                                                                                                         |
